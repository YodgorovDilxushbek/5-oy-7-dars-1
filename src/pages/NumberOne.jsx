...
const NumberOne = () => {
  ...
  <div className="flex gap-8 mt-4">
    <label className="select-none cursor-pointer flex items-center gap-2 text-gray-800 font-medium" htmlFor="usd">
      USD
      <input 
        id="usd" 
        name="checked" 
        value={'USD'} 
        onChange={(e) => { setCheckedValute(e.target.value); }} 
        type="radio" 
        checked={checkedValute === 'USD'} 
        className="accent-blue-500" 
      />
    </label>
    <label className="select-none cursor-pointer flex items-center gap-2 text-gray-800 font-medium" htmlFor="eur">
      EUR
      <input 
        id="eur" 
        name="checked" 
  ...
  <form className="flex gap-4 bg-blue-500 max-w-[350px] w-full rounded-md shadow-md" onSubmit={handleConvert}>
    <input 
  ...
  <h1 className="text-gray-600 border px-3 py-1 shadow-cyan-500/ shadow-lg  text-[20px] font-semibold mt-4">{result}</h1>
</div>
